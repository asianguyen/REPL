...
public class TestSearchCSV {
  ...
  //     HttpURLConnection connection2 = tryRequest("search?value=Black");
  //     assertEquals(200, connection.getResponseCode());
  ...
  //     array.add("Black");
  //     array.add(" $770.26 ");
  //     array.add("30424.80376");
  //     array.add(" $0.73 ");
  //     array.add("6%]");

  //     assertEquals(array, responseBody2.get("data"));
  ...
  public void testSearchNoParams() throws IOException {
    HttpURLConnection connection = tryRequest("search");
    assertEquals(200, connection.getResponseCode());
    Map<String, Object> responseBody =
        this.adapter.fromJson(new Buffer().readFrom(connection.getInputStream()));
    assertEquals("value_not_entered", responseBody.get("result"));
    connection.disconnect();
  }
  ...
  public void testSearchIndex() throws IOException {
    ...
    HttpURLConnection connection2 = tryRequest("search?value=Black&column=1");
    assertEquals(200, connection.getResponseCode());
    // Map<String, Object> responseBody2 =
    //     this.adapter.fromJson(new Buffer().readFrom(connection2.getInputStream()));
